...
def test_theorem_c_base_unit_identities():
    ...
    assert np.isclose(T0, hbar/(m_e*c**2), rtol=1e-12), f"T0 = {T0} ≠ {hbar/(m_e*c**2)}"
...
def test_theorem_b_g_monomial():
    ...
    assert np.allclose(actual_exponents, expected_exponents, rtol=1e-12), \
    ...
    assert np.isclose(g_unit, g_expected, rtol=1e-12), \
...
def test_theorem_a_dimensional_homomorphism():
    ...
    assert np.isclose(uELT, uE*uL*uT, rtol=1e-12), \
    ...
    homomorphism_verified = np.isclose(u_energy_plus_momentum, u_energy * u_momentum, rtol=1e-12)
    assert homomorphism_verified, \
...
def test_theorem_e_thomas_wigner():
    ...
    assert orthogonality_error_near_c < 1e-10, f"Near-c orthogonality error: {orthogonality_error_near_c}"
...
def test_basis_necessity_theorem():
    ...
    assert np.isclose(base_partial["M0"], m_tiny, rtol=1e-12), f"Mass scale should be {m_tiny}, got {base_partial['M0']}"
    assert base_partial["L0"] < 1e-10, f"Length scale should be small for heavy particle, got {base_partial['L0']}"
    assert base_partial["T0"] < 1e-18, f"Time scale should be small for heavy particle, got {base_partial['T0']}"
    assert np.isclose(base_partial["L0"] / base_partial["T0"], c, rtol=1e-12), "c-invariance should be preserved"
    
    print("✓ Basis Necessity Theorem: {ħ, c, m⋆} basis is necessary (tested with extreme scaling)")
    return True
