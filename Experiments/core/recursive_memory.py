...
class MemoryPath:
    ...
    def __post_init__(self):
        ...
        if not (len(self.points) == len(self.velocities) == 
                len(self.phases) == len(self.stage_indicators)):
            raise ValueError("All path components must have same length")
    
    def add_step(self, point: np.ndarray, velocity: np.ndarray, 
                 phase: complex, stage: str):
...
class RecursiveMemory:
    ...
    def determine_stage(self, velocity: np.ndarray, 
    ...
    def compute_stage_phase(self, stage: str, velocity: np.ndarray, 
                           context: Optional[Dict[str, Any]] = None) -> complex:
        """
    ...
    def build_recursive_path(self, initial_point: np.ndarray,
                           initial_velocity: np.ndarray,
                           n_steps: int,
                           step_function: Optional[Callable] = None) -> MemoryPath:
        """
        ...
        if n_steps > self.memory_depth:
            warnings.warn(f"Path length {n_steps} exceeds memory depth {self.memory_depth}")
        ...
        try:
            for step in range(1, n_steps):
                ...
                if step_function is not None:
                    next_point, next_velocity = step_function(current_point, current_velocity, step)
                else:
                    ...
                    try:
                        ...
                        warnings.warn(f"Gyroaddition failed at step {step}, using fallback: {e}")
    ...
    def estimate_kappa_from_geometry(self, n_paths: int = 10, 
                                   path_length: int = 50) -> Dict[str, Any]:
        """
        ...
        if monodromy_magnitude > self.coherence_threshold:
            kappa_estimate = (coherence_magnitude * gradient_magnitude) / monodromy_magnitude
        else:
    ...
    def run_memory_experiment(self, n_paths: int = 20, 
                            path_length: int = 100) -> Dict[str, Any]:
        """
        ...
        print(f"  Average monodromy magnitude: {avg_monodromy:.6f} ± {std_monodromy:.6f}")
