...
class CGMAcousticDiagnostics:
    ...
    def sound_speed_cgm_enhanced(self, gyration_angle: float, 
                                translation_phase: float, z: float | None = None) -> float:
        """
    ...
    def visibility_weight_toroidal_enhanced(self, nhat, axis=np.array([0, 0, 1])):
        ...
        """
        tau_rms = 1e-3
        tau = tau_from_template(unit(nhat), tau_rms=tau_rms,
                               axis=axis, 
    ...
    def map_gyration_to_acoustic_modes_enhanced(self, gyration_sequence: List[float],
                                               use_holonomy: bool = True) -> Dict[str, Any]:
        """
    ...
    def test_phase_coherence_enhanced(self, acoustic_modes: List[Dict], 
                                     compression_stride: int = 4) -> Dict[str, Any]:
        """
    ...
    def confront_peak_locations(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "overall_passed": all(c["passes"] for c in comparisons)  # require all peaks to pass
        }
    
    def test_odd_even_modulation(self, gyration_sequence: List[float]) -> Dict[str, Any]:
    ...
    def run_diagnostic_suite(self) -> Dict[str, Any]:
        ...
        print(f"   max CGM c_s: {ss['max_cgm_c_s']:.3e} m/s at θ={ss['best_angles']['theta']:.3f}, φ={ss['best_angles']['phi']:.3f} {status}")
        ...
        for comp in peak_results["comparisons"]:
            ...
            print(f"   Peak {comp['peak']}: ℓ={comp['predicted']:.0f} "
        ...
        print(f"   Modulation significance: {modulation['modulation_significance']:.3f} {status}")
        ...
        overall_pass = sum(assessments.values()) == 4  # Pass ONLY if ALL 4 acoustic tests pass (truly rigorous)
