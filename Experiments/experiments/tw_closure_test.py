...
class TWClosureTester:
    ...
    def _signed_rotation_angle(self, R: np.ndarray, normal=np.array([0.,0.,1.])) -> float:
    ...
    def solve_beta_for_mp(self) -> float:
        ...
        for _ in range(20):
            ...
            if abs(cur - self.m_p) < 1e-12:
                break
            db = 1e-6
            dcur = (self.wigner_angle_exact(beta+db, self.o_p) -
                    self.wigner_angle_exact(beta-db, self.o_p)) / (2*db)
            if abs(dcur) < 1e-12: 
                break
    ...
    def bu_pole_asymmetry(self) -> Dict[str, float]:
        ...
        return {
            "egress_plus": a1, "ingress_plus": b1,
            "egress_minus": a2, "ingress_minus": b2,
            "egress_plus_signed": a1s, "ingress_plus_signed": b1s,
            "egress_minus_signed": a2s, "ingress_minus_signed": b2s,
    ...
    def estimate_thomas_curvature(self, beta0=None, theta0=None, dβ=1e-3, dθ=1e-3, grid=5) -> Dict[str, float]:
        """
        Discrete curvature proxy F_{βθ} ≈ ∂θ ω - ∂β ω
        sampled on a small (β,θ) grid centered at (beta0, theta0).
        """
        if beta0 is None: beta0 = self.u_p
        if theta0 is None: theta0 = self.o_p
