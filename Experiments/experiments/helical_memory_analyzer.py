...
class HelicalMemoryAnalyzer:
    ...
    def _compute_cs_evolution(self) -> Dict[str, Any]:
        ...
        alpha = np.pi / 4
        cs_operator = np.cos(alpha/2) * self.identity - 1j * np.sin(alpha/2) * self.sigma_3
    ...
    def _compute_una_evolution(self, cs_evolution: Dict[str, Any]) -> Dict[str, Any]:
        ...
        beta = np.pi / 8
        una_operator = cs_evolution['operator'] @ (np.cos(beta/2) * self.identity + 1j * np.sin(beta/2) * self.sigma_1)
    ...
    def _compute_ona_evolution(self, una_evolution: Dict[str, Any]) -> Dict[str, Any]:
        ...
        gamma = np.pi / 8
        ona_operator = una_evolution['operator'] @ (np.cos(gamma/2) * self.identity + 1j * np.sin(gamma/2) * self.sigma_2)
    ...
    def _compute_bu_evolution(self, ona_evolution: Dict[str, Any]) -> Dict[str, Any]:
        ...
        delta = np.pi / 8
        bu_operator = ona_evolution['operator'] @ (np.cos(delta/2) * self.identity + 1j * np.sin(delta/2) * self.sigma_3)
    ...
    def _compute_helical_psi_bu_field(self, evolution_progression: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        ...
        spin_translation_coherence = np.mean(evolution_phases) if evolution_phases else 1.0
        ...
        res_loop = phi_loop / (2.0 * np.pi)  # 0 = perfect closure, small positive otherwise
    ...
    def _map_helical_to_cosmic_scales(self, psi_bu: Dict[str, Any] | None = None, predict_cmb: bool = False) -> Dict[str, Any]:
        """
        ...
        if predict_cmb:
            ...
            if psi_bu is None:
                ...
                spin_translation_coherence = max(1e-6, psi_bu.get("spin_translation_coherence", 1e-6))
            ...
            print(f"   Deviation: {abs(cmb_temp_predicted - self.T_cmb_measured)/self.T_cmb_measured*100:.1f}%")
            print(f"   N* = {N_star}, N_measured = {N_measured}, ladder_ratio = {ladder_ratio:.3f}")
    ...
    def _test_helical_hypotheses(self, cosmic_scales: Dict[str, Any], psi_bu: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        de_error = abs(cosmic_scales['dark_energy_predicted'] - expected_de_density) / expected_de_density
        tests["dark_energy"] = {
    ...
    def emergent_length_from_full_loop(self, N: int, psi_bu: Dict[str, Any] | None = None, base_length: float = 3.861592e-13) -> Dict[str, float]:
        """
        ...
        return {"L_star_N": float(Lstar), "pitch_loop": loop["pitch_loop"], "Xi_loop": float(Xi)}

    def solve_min_N_for_CMB(self, psi_bu: Dict[str, Any] | None = None, Nmax: int = 500) -> tuple[int, Dict[str, float]]:
        """
    ...
    def solve_N_for_target_length(self, L_target: float, psi_bu: Dict[str, Any] | None = None,
                                  Nmax: int = 200) -> tuple[int, Dict[str, float]]:
        """
    ...
    def report_bio_bridge(self, psi_bu: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        if out["dna_pitch_m"]["N"] > 0 and out["dna_rise_m"]["N"] > 0:
            out["bp_per_turn_pred"] = out["dna_pitch_m"]["L_star_N"] / out["dna_rise_m"]["L_star_N"]
        else:
            out["bp_per_turn_pred"] = None
            
        return out

    def fit_single_Xi_for_targets(self, targets: dict[str, float], Ns: dict[str, int],
                                  psi_bu: Dict[str, Any] | None = None) -> dict:
        """
        ...
        for Xi in Xi_grid:
            ...
            if best is None or sse < best["sse"]:
                best = {"Xi": float(Xi), "sse": sse, "errors": {k: errs[i] for i, k in enumerate(targets)}}
        ...
        return {"Xi_bio": best["Xi"], "sse": best["sse"], "predicted": pred, "ratios": ratios,
                "bp_per_turn_pred": bp_per_turn, "Ns_used": Ns, "Π_loop": Π}

    def fit_bio_joint(self, targets: Dict[str, float],
                      Xi_range: Tuple[float, float] = (0.95, 1.25),
                      Xi_steps: int = 6000) -> Dict[str, Any]:
        """
        ...
        for Xi in np.linspace(Xi_range[0], Xi_range[1], Xi_steps):
            ...
            if best is None or sse < best["sse"]:
                best = {"Xi_bio": float(Xi), "Ns": Ns, "predicted": preds, "sse": sse, "Π": Π}

        # bp/turn = pitch / rise prediction
        if best:
            bp_per_turn = best["predicted"]["dna_pitch_m"] / best["predicted"]["dna_rise_m"]
            best["bp_per_turn_pred"] = float(bp_per_turn)
        return best or {"Xi_bio": None, "Ns": {}, "predicted": {}, "sse": float("inf"), "Π": Π}
    ...
    def test_bio_bridge_out_of_sample(self, Xi_bio: float) -> Dict[str, Any]:
        ...
        for scale_name, target_length in out_of_sample_targets.items():
            ...
            print(f"  {scale_name:20s}: N={N_pred:2d}, pred={pred_length:.2e} m, ratio={ratio:.3f} {status}")
    ...
    def test_chirality_selection(self, psi_bu: Dict[str, Any] | None = None) -> Dict[str, Any]:
        ...
        for axis_name, flip_matrix in flip_axes.items():
            ...
            U_flip = np.cos(np.pi/2)*self.identity + 1j*np.sin(np.pi/2)*flip_matrix
        ...
        preferred_axis = max(chirality_results.keys(), 
        ...
        print(f"   Current choice (σ₁): {chirality_results['sigma_1']['chirality_code']}")
        ...
        if chirality_consistent:
            ...
            print(f"   ⚠️  CHIRALITY VARIABLE: Different axes give different preferences")
        ...
        print(f"\n📝 INTERPRETATION:")
        print(f"   Model predicts a right-handed sugar preference; by biochemical convention")
        print(f"   that corresponds to D-sugars (and typically L-amino acids for proteins).")
    ...
    def run_comprehensive_analysis(self) -> Dict[str, Any]:
        ...
        print("=" * 60)
        print("Testing hypothesis: DNA helix scales emerge from same loop pitch as cosmic scales")
        ...
        for k, v in bio_results.items():
            if k == "bp_per_turn_pred":
                ...
                if v['N'] > 0:
                    print(f"  {k}: N={v['N']}, L*(N)={v['L_star_N']:.3e} m, target={v['L_target']:.3e} m")
                else:
                    print(f"  {k}: N not found in range, best L*={v['L_star_N']:.3e} m, target={v['L_target']:.3e} m")
        ...
        bio_Ns = {k: bio_results[k]["N"] for k in ["dna_rise_m","dna_pitch_m","dna_diam_m"]}
        try:
            ...
            print(f"  predicted bp/turn (pitch/rise): {bio_fit['bp_per_turn_pred']:.2f}")
            ...
            if "dna_pitch_m" in bio_Ns and "dna_rise_m" in bio_Ns:
                ...
                print(f"  bp/turn ≈ (Π/Xi)^{delta_N} = {bio_fit['bp_per_turn_pred']:.2f}")
        ...
        try:
            ...
            if bio_joint["Xi_bio"] is not None:
                ...
                print(f"  predicted bp/turn (pitch/rise): {bio_joint['bp_per_turn_pred']:.2f}")
            else:
        ...
        if bio_joint["Xi_bio"] is not None:
            bio_out_of_sample_results = self.test_bio_bridge_out_of_sample(bio_joint["Xi_bio"])
        ...
        passed_tests = sum(1 for test in results['hypothesis_tests'].values() if test['passed'])
        ...
        if 'cmb_prediction' in results:
            ...
            print(f"  CMB Prediction: {cmb_status} ({cmb_pred['deviation']*100:.1f}% deviation)")
        ...
        if 'bio_out_of_sample' in results:
            ...
            if 'error' not in bio_out:
                ...
                print(f"  Bio-Bridge Out-of-Sample: {bio_status} ({bio_out['passed_tests']}/{bio_out['total_tests']} tests)")
            else:
        ...
        if chirality_results['chirality_consistent']:
            print(f"🎯 CHIRALITY STABLE: All axes give same preference: {chirality_results['preferred_chirality']['chirality']}")
        else:
    ...
    def _emergent_length_scale(self, psi_bu: Dict[str, Any], base_length: float = 3.861592e-13) -> float:
        """
