        ...
        if si_unit > 0:
            ...
            if dimensions.L == 1 and dimensions.M == 0 and dimensions.T == 0:
                if si_unit < 1e-12 or si_unit > 1e6:  # nm to km
                    audit["warning"] = f"Length {si_unit:.2e} m may be outside reasonable range"
            # For time: should be reasonable (fs to years range)  
                ...
                if si_unit < 1e-18 or si_unit > 1e9:  # fs to years
                    audit["warning"] = f"Time {si_unit:.2e} s may be outside reasonable range"
            # For mass: should be reasonable (ng to kg range)
                ...
                if si_unit < 1e-15 or si_unit > 1e3:  # ng to kg
                    audit["warning"] = f"Mass {si_unit:.2e} kg may be outside reasonable range"
    
    return audit


def validate_dimensions(calibrator: DimensionalCalibrator, 
                       expected: DimVec, actual: DimVec, 
