...
class InterferenceTest:
    """Test for interference patterns from inside a 98% closed toroidal structure."""
...
class CrossScaleValidator:
    ...
    def test_planck_y_map(
        ...
        """Test interference pattern from inside a 98% closed toroidal structure."""
        ...
        try:
            from astropy.coordinates import SkyCoord, Galactic, ICRS, GeocentricTrueEcliptic  # pyright: ignore[reportMissingImports]
            ...
            ncp_icrs = SkyCoord(ra=0*u.deg, dec=90*u.deg, frame=ICRS)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue,reportUnknownMemberType]
            ncp_gal = ncp_icrs.transform_to(Galactic)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue]
            ncp_l = float(ncp_gal.l.rad)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue,reportOptionalMemberAccess,reportArgumentType]
            ncp_b = float(ncp_gal.b.rad)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue,reportOptionalMemberAccess,reportArgumentType]
            ncp = np.array([np.cos(ncp_b)*np.cos(ncp_l), np.cos(ncp_b)*np.sin(ncp_l), np.sin(ncp_b)])
            
            # North Ecliptic Pole: transform (λ=any, β=+90°) ecliptic → ICRS → Galactic
            nep_ecl = SkyCoord(lon=0*u.deg, lat=90*u.deg, frame=GeocentricTrueEcliptic)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue]
            nep_icrs = nep_ecl.transform_to(ICRS)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue]
            nep_gal = nep_ecl.transform_to(Galactic)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue]
            nep_l = float(nep_gal.l.rad)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue,reportOptionalMemberAccess,reportArgumentType]
            nep_b = float(nep_gal.b.rad)  # pyright: ignore[reportGeneralTypeIssues,reportAttributeAccessIssue,reportOptionalMemberAccess,reportArgumentType]
            nep = np.array([np.cos(nep_b)*np.cos(nep_l), np.cos(nep_b)*np.sin(nep_l), np.sin(nep_b)])
            
            print(f"    Coordinate transforms: NCP(l={np.degrees(ncp_l):.1f}°, b={np.degrees(ncp_b):.1f}°)")
            print(f"    NEP(l={np.degrees(nep_l):.1f}°, b={np.degrees(nep_b):.1f}°)")
            print(f"    Literature values: Earth obliquity=23.44°, Ecliptic-Galactic=60.19°, CMB-ecliptic=9.35°")
            ...
            ncp = np.array([0.0, 0.0, 1.0])  # Will be transformed by astropy if available
            ...
            print(f"    Literature-sourced axes: Earth obliquity=23.44°, Ecliptic-Galactic=60.19°")
        ...
        for i in range(100):
            ...
            axis_rand = np.array([np.cos(b_rand)*np.cos(l_rand), np.cos(b_rand)*np.sin(l_rand), np.sin(b_rand)])
        ...
        print(f"    CMB dipole rank: {cmb_rank}/{len(random_Ts)+1} (p={cmb_p_random:.4f})")
...
def main():
    """Run interference pattern analysis - testing the 98% closed toroid hypothesis."""
    ...
    print("Testing: We're inside a 98% closed toroidal structure")
    ...
    if 'cmb_axis_rank' in cmb_result:
        print(f"  CMB dipole axis preference: rank {cmb_result['cmb_axis_rank']}/101 (p={cmb_result['cmb_axis_p_random']:.4f})")
    ...
    axis_pref_sig = cmb_result.get("cmb_axis_p_random", 1.0) < 0.05  # p < 0.05 for significant evidence
    ...
    if p2c4_sig and ladder_sig and phase_lock_sig and sn_sig and bao_sig:
        ...
        if axis_pref_sig:
            print("  Additional support: CMB dipole axis shows preference over other cosmic reference frames.")
    else:
        print(
