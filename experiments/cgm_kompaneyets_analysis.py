...
class CGMKompaneyetsAnalyzer:
    ...
    def __init__(
        self,
        gyrospace: Optional[GyroVectorSpace] = None,
    ...
    def predict_y_sky(
        ...
        for i, th in enumerate(thetas):
            for j, ph in enumerate(phis):
                ...
                dist = self.distort_by_energy_injection(
                    fe, T_e, n_e, dt, use_sources=use_sources
                )
    ...
    def validate_against_firas(self, distortions: Dict[str, float]) -> Dict[str, Any]:
        ...
        mu_margin = (
            self.mu_firas_limit / abs(mu_eff) if abs(mu_eff) > 0 else float("inf")
        )
    ...
    def demonstrate_two_regimes(self) -> Dict[str, Any]:
        ...
        print(
            f"   μ within FIRAS: {'✅' if abs(final_fit1['mu_effective']) < self.mu_firas_limit else '❌'}"
        )
        print(
            f"   y within FIRAS: {'✅' if abs(final_fit1['y_effective']) < self.y_firas_limit else '❌'}"
        )
        ...
        print(
            f"   μ within FIRAS: {'✅' if abs(final_fit2['mu_effective']) < self.mu_firas_limit else '❌'}"
        )
        print(
            f"   y within FIRAS: {'✅' if abs(final_fit2['y_effective']) < self.y_firas_limit else '❌'}"
        )
    ...
    def calibrate_with_tsz_identity(
        self, energy_injection: float, final_y: float
    ) -> Dict[str, Any]:
        """
    ...
    def analyze_cgm_white_to_black(
...
def main():
    ...
    for energy_injection in energy_injections:
        ...
        calibration = analyzer_enhanced.calibrate_with_tsz_identity(
            energy_injection, simulated_y
        )
