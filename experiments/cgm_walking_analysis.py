...
class CGMWalkingTheory:
    ...
    def analyze_aperture_correspondence(self) -> Dict:
        ...
        return {
            ...
            'defect_ratio_maneuver_to_steady': walking_defect_maneuver / walking_defect_steady,
            'R2_ML': R2_ML,
            'R2_AP': R2_AP,
            'R2_avg': R2_avg,
            ...
            'defect_prediction_error_ML': abs(predicted_defect_ML - walking_defect_steady) / walking_defect_steady,
            'defect_prediction_error_AP': abs(predicted_defect_AP - walking_defect_steady) / walking_defect_steady,
            'defect_prediction_error_avg': abs(predicted_defect_avg - walking_defect_steady) / walking_defect_steady
        }
    ...
    def moment_sign_for_recovery(self, x_CM, x_CP, z_CM, Fz) -> Dict:
        ...
        required_Ty_sign = np.sign(z_CM * zero_moment_term)  # this must be positive to subtract
    ...
    def analyze_information_entropy(self) -> Dict:
        ...
        states = [p_stance * (1-p_transition), 
        ...
        H_cgm = -self.cgm.m_p * np.log2(self.cgm.m_p) - (1-self.cgm.m_p) * np.log2(1-self.cgm.m_p)
    ...
    def analyze_action_quantization(self) -> Dict:
        ...
        quantization_error = abs(action_quanta - nearest_int) / nearest_int if nearest_int > 0 else 1
    ...
    def analyze_intelligence_metrics(self) -> Dict:
        ...
        intelligence_metric = np.sqrt(compression_efficiency * self.walking.cmp_cp_steady)
        
        # Compare to CGM theoretical optimum
        cgm_optimum = np.sqrt(0.5 * self.cgm.m_p)  # geometric mean of 50% compression and aperture
    ...
    def analyze_beam_walk_constraints(self, beam_width: float = 0.03, foot_length: float = 0.25) -> Dict:
        """
        ...
        required_factor = bos_half_beam / defect_m  # <1 means you must reduce the defect
        ...
        return {
            ...
            'required_defect_percent': 100 * self.walking.cmp_cp_steady * required_factor
        }
    ...
    def analyze_alignment_intelligence(self) -> Dict:
        ...
        alignment_intelligence = {
            ...
            'alignment_intelligence_score': float(np.prod(components) ** (1/len(components)))
        }
    ...
    def compute_unified_compatibility(self, all_results: Dict) -> Dict:
        ...
        time_match = 1 / (1 + abs(ratio - nearest_multiple))  # peaks at integer multiples
...
def run_complete_analysis():
    ...
    moment_test = analyzer.moment_sign_for_recovery(x_CM=0.1, x_CP=0.05, z_CM=1.0, Fz=700)
    ...
    print(f"  Cancellation: {walking.cancellation_ML*100:.0f}% ML, "
          f"{walking.cancellation_AP*100:.0f}% AP, {walking.cancellation_V*100:.0f}% V")
    print(f"  Zero-moment R2: {walking.R2_x_steady:.2f} (x), {walking.R2_y_steady:.2f} (y)")
    print(f"  CMP-CP defect: {walking.cmp_cp_steady*100:.0f}% steady, "
    ...
    print(f"Walking defect (steady): {aperture_results['walking_defect_steady']*100:.1f}%")
    print(f"Defect ratio (maneuver/steady): {aperture_results['defect_ratio_maneuver_to_steady']:.2f}")
    print(f"Zero-moment R2: ML={aperture_results['R2_ML']:.2f}, AP={aperture_results['R2_AP']:.2f}")
    print(f"Residual from model: ML={aperture_results['residual_ML']*100:.1f}%, AP={aperture_results['residual_AP']*100:.1f}%")
    print(f"Defect prediction: ML={aperture_results['predicted_defect_ML']*100:.1f}%, AP={aperture_results['predicted_defect_AP']*100:.1f}%")
    print(f"Combined prediction: {aperture_results['predicted_defect_avg']*100:.1f}% (error: {aperture_results['defect_prediction_error_avg']*100:.1f}%)")
    ...
    print(f"DS required for 120°: {phase_results['double_support_required_for_120']*100:.1f}%")
    print(f"DS gap: {phase_results['double_support_gap']*100:.1f}%")
    print(f"Residual after cancellation: ML={phase_results['residual_ML']*100:.1f}%, "
    ...
    print(f"Axis fractions: ML={obs_results['axis_fractions'][0]:.3f}, AP={obs_results['axis_fractions'][1]:.3f}, V={obs_results['axis_fractions'][2]:.3f}")
    print(f"Anisotropy: {obs_results['anisotropy']:.3f} (norm: {obs_results['anisotropy_norm']:.3f})")
    ...
    print(f"Defect AP: {perp['defect_ap_m']*100:.1f} cm  |  BOS half-length: {perp['bos_half_length_m']*100:.1f} cm")
    print(f"Defect ML: {perp['defect_ml_m']*100:.1f} cm  |  BOS half-width: {perp['bos_half_width_m']*100:.1f} cm")
    print(f"Perpendicularity margin AP: {perp['perp_margin_ap']:.3f}  (1=fully safe, 0=edge)")
    print(f"Perpendicularity margin ML: {perp['perp_margin_ml']:.3f}  (1=fully safe, 0=edge)")
    ...
    print(f"Helicity sign: {'right-handed' if chir['helicity_sign']>0 else 'left-handed'}")
    print(f"Ortho score: {chir['ortho_score']:.3f}  |  DS score: {chir['ds_score']:.3f}")
    ...
    results['beam'] = beam
    print(f"Required defect factor: {beam['required_defect_factor']:.3f}  → "
    ...
    results['ageing'] = aged
    print(f"Aged axis fractions ML/AP/V: "
    ...
    print(f"Horizon distance (steady): {thermo_results['horizon_distance_steady']*100:.1f}%")
    print(f"Info leakage rate: {thermo_results['info_leakage_rate']:.4f}")
    print(f"Aperture efficiency (m_p²×Q_G): {thermo_results['aperture_efficiency']:.4f}")
    print(f"Closure identity: {'OK' if thermo_results['closure_identity_ok'] else 'FAIL'}")
    ...
    print(f"Minimum description length: {intelligence_results['minimum_description_length']:.4f} bits")
    print(f"Compression efficiency: {intelligence_results['compression_efficiency']:.4f}")
    ...
    print(f"  Helical Structure: {alignment_results['helical_structure']:.3f} (left-handed chirality)")
    print(f"  Perpendicularity Margin AP: {alignment_results['perpendicularity_margin_ap']:.3f}")
    print(f"  Perpendicularity Margin ML: {alignment_results['perpendicularity_margin_ml']:.3f}")
    print(f"  Overall Margin: {alignment_results['perpendicularity_margin_overall']:.3f}")
    print(f"  Action Quanta: {alignment_results['action_quanta']:.3f} (quantized corrections)")
    print(f"  Phase Coherence: {alignment_results['phase_coherence']:.3f} (120° coordination)")
    ...
    print(f"  through quantized helical corrections - the essence of physical intelligence")
