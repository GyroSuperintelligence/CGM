...
class ToroidalCoherenceTest:
    ...
    def precompute(self, nside: int, mask: np.ndarray):
        ...
        templates = []
        for ax in axes:
            templates.append(generate_toroidal_template(nside, ax,
                             self.thresholds.a_polar, self.thresholds.b_cubic).astype(np.float32))
        v = (mask > 0)
    ...
    def null_distribution_phase_randomized(self, data_map: np.ndarray,
                                          ...
                                          seed: int = 42, n_jobs=None,
                                          alm_precomputed=None) -> List[Dict]:
    ...
    def null_distribution_p2c4(self, data_map, mask, nside, lmax, axis, n_mc=1000, seed=42, n_jobs=None, alm_precomputed=None):
...
class CrossScaleValidator:
    ...
    def test_planck_y_map(self, memory_axis: np.ndarray) -> Dict[str, Any]:
        ...
        data = self.dm.get_planck_data(nside=nside, lmax=lmax, fwhm_deg=fwhm, fast_preprocess=True)
        ...
        for label, ring_phase, inside in [
            ...
            amps = test.measure_lobe_amplitudes(data['y_map'], data['mask'], data['nside'])
        ...
        best_test, best_coh = max(configs, key=lambda tc: abs(tc[1]['amplitude_coherence']))
        print(f"  Best config: {best_coh['label']} (inside_view={best_coh['inside_view']})")
        ...
        null_dist = best_test.null_distribution_phase_randomized(
            data['y_map'], data['mask'], data['nside'], data['lmax'],
        ...
        print(f"  Planck p-values: sign={p_values['sign_coherence']:.4f}, amp={p_values['amplitude_coherence']:.4f}")

        # P2/C4 joint amplitude test
        from functions.torus import project_P2_C4_healpix
        pc_obs = project_P2_C4_healpix(data['y_map'], data['mask'], data['nside'], axis=memory_axis)
        a2, a4 = float(pc_obs["a2"]), float(pc_obs["a4"])
        
        # Null for (a2,a4)
        coefs_null = best_test.null_distribution_p2c4(
            data['y_map'], data['mask'], data['nside'], data['lmax'],
            axis=memory_axis, n_mc=int(os.getenv("CGM_MC", "1000")),
            seed=42, n_jobs=max(1, (os.cpu_count() or 4)//2), alm_precomputed=data['alm']
        )
        ...
        l37_ratio = float(l37 / (neighborhood + 1e-30)) if np.isfinite(l37) else float('nan')

        # P2/C4 anatomy (CGM-style)
        try:
            from functions.torus import project_P2_C4_healpix
            pc = project_P2_C4_healpix(data['y_map'], data['mask'], data['nside'], axis=memory_axis)
            print(f"  P2/C4 projection (masked): frac P2={pc['frac_power_P2']:.3f}, frac C4={pc['frac_power_C4']:.3f}")
        except ImportError:
        ...
        result = {
            ...
            'p2_c4': pc,
            'p2c4_a2': a2, 'p2c4_a4': a4,
            'p2c4_T': T_obs, 'p2c4_p': p_T
        }
        print(f"  ℓ=37 power ratio: {l37_ratio:.3f} (1.0 = no feature)")
        
        if best_coh['label'] == "inside_rot":
            print("  → Inside-view toroid matches best. Observer-inside (BU-node) geometry supported.")
    ...
    def test_supernova_residuals(self, memory_axis: np.ndarray) -> Dict[str, Any]:
        ...
        C4 = positions[:,0]**4 + positions[:,1]**4 + positions[:,2]**4 - 3/5  # cubic C4
        ...
        print(f"  Best view: {best_view}, Sign coherence: {sign_coherence:.3f} (p={p_value:.4f})")
    ...
    def test_cross_scale_consistency(self) -> Dict[str, Any]:
        ...
        for name, axis in axes_to_test.items():
            ...
            stats["l37_ratios"].append(axis_results['planck'].get('l37_ratio_local', np.nan))
        ...
        print("\nDISCOVERY SUMMARY")
        print("-----------------")
        print(f"  Holonomy rotation preferred on {counts.get('holonomy_rot',0)}/{len(stats['best_label'])} axes")
        print(f"  Inside-view preferred on {counts.get('inside_rot',0)}/{len(stats['best_label'])} axes")
        print(f"  ℓ=37 mean ratio: {np.nanmean(stats['l37_ratios']):.3f}")
        print(f"  P2 mean fraction: {np.nanmean(stats['p2']):.3f}, C4 mean fraction: {np.nanmean(stats['c4']):.3f}")
