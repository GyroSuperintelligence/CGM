...
def su3_hadron_checks():
    ...
    print("\nCGM SU(3) HADRON ANALYSIS")
    print("="*50)
    print("SU(3)_flavor octet/decuplet checks for CGM geometric patterns in hadron masses.")
    print("This analysis looks for CGM invariants in hadron mass relations as a probe of")
    ...
    print(f"  spacings: {[f'{s:.3f}' for s in spacings]} GeV, mean={mean_spacing:.3f} GeV")
    print(f"  frac devs: {[f'{d*100:.2f}%' for d in frac_devs]}   (Δ = {Delta*100:.2f}%)")
    ...
    print(f"Coleman-Glashow isospin: (Ξ⁻ − Ξ⁰) = {Delta_Xi*1000:.1f} MeV, (Σ⁻ − Σ⁺) = {Delta_Sigma*1000:.1f} MeV")
    print(f"  Fractional defect: {((Delta_Xi + Delta_Sigma)/2/0.001 - 1)*100:.3f}%  (EM/isospin breaking ~1 MeV)")
