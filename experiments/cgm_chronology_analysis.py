...
def compute_generalized_rayleigh(fractional_parts: List[float], m: int = 2) -> Dict[str, float]:
    """
    ...
    if not fractional_parts:
        return {"p_value": 1.0, "concentration": 0.0, "n_events": 0, "m": m, "circular_mean": 0.0}
...
def compute_spacing_test(milestone_times: List[float], t_planck: float, m_p: float) -> Dict[str, float]:
    """
    ...
    delta_n_values = [math.log(times[i] / times[i-1], 1.0/m_p) for i in range(1, len(times))]
...
def kuiper_test(fractional_parts: List[float], n_bootstrap: int = 1_000) -> Dict[str, float]:
    """
    ...
    p_value = sum(1 for stat in bootstrap_stats if stat >= kuiper_stat) / len(bootstrap_stats)
...
def watson_test(fractional_parts: List[float], n_bootstrap: int = 1_000) -> Dict[str, float]:
    """
    ...
    bootstrap_stats.sort()
    p_value = sum(1 for stat in bootstrap_stats if stat >= u_squared) / len(bootstrap_stats)
...
def compute_cgm_composite_harmonic_test(fractional_parts: List[float]) -> Dict[str, float]:
    """
...
def monte_carlo_analysis_time(
...
) -> Tuple[List[ResultRow], Dict[str, Union[float, str]], List[Dict[str, Dict[str, float]]]]:
    ...
    if exclude_anchors:
        test_milestones = [m for m in milestones if m.name not in ["Planck time", "Today"]]
        anchor_milestones = [m for m in milestones if m.name in ["Planck time", "Today"]]
    else:
    ...
    test_types = ["rayleigh_m2", "rayleigh_m3", "rayleigh_m4", "cgm_composite", "weighted_cgm", "spacing_test", "kuiper", "watson"]
    for test_type in test_types:
        p_values = [s[test_type]["p_value"] for s in test_stats if test_type in s]
        concentrations = [s[test_type].get("concentration", 0.0) for s in test_stats if test_type in s]
        
        if p_values:
            aggregated_stats[f"{test_type}_mean_p"] = sum(p_values) / len(p_values)
            aggregated_stats[f"{test_type}_median_p"] = sorted(p_values)[len(p_values) // 2]
            aggregated_stats[f"{test_type}_mean_concentration"] = sum(concentrations) / len(concentrations)
        else:
...
def run_comprehensive_chronology_analysis() -> None:
    ...
    results_time, stats_time, test_stats_time = monte_carlo_analysis_time(
        milestones_time, t0_cgm, m_p, 
        n_samples=10_000, exclude_anchors=True, rng=rng
    )
    ...
    results_temp, stats_temp, test_stats_temp = monte_carlo_analysis_temperature(
        milestones_temp, t_planck_gev, m_p, 
        n_samples=10_000, exclude_anchors=True, rng=rng
    )
    ...
    results_scale, stats_scale, test_stats_scale = monte_carlo_analysis_scale_factor(
        milestones_scale, a_reference, m_p, 
        n_samples=10_000, exclude_anchors=True, rng=rng
    )
    ...
    results_temp_sqrt3, stats_temp_sqrt3, test_stats_temp_sqrt3 = monte_carlo_analysis_temperature(
        milestones_temp, t_planck_gev, 1.0/lambda_sqrt3, 
        n_samples=10_000, exclude_anchors=True, rng=rng
    )
    ...
    print(f"{'FRW Time (T0_CGM)':<20} {primary_p_time:<15.4f} {'Null' if primary_p_time > 0.05 else 'Significant'}")
    print(f"{'Temperature (1/m_p)':<20} {primary_p_temp:<15.4f} {'Null' if primary_p_temp > 0.05 else 'Significant'}")
    print(f"{'Temperature (√3)':<20} {primary_p_temp_sqrt3:<15.4f} {'Null' if primary_p_temp_sqrt3 > 0.05 else 'Significant'}")
    print(f"{'Scale Factor':<20} {primary_p_scale:<15.4f} {'Null' if primary_p_scale > 0.05 else 'Significant'}")
    print()
    ...
    if best_p < 0.05:
        if primary_p_temp_sqrt3 == best_p:
            print("INTERPRETATION: Evidence for CGM chronology in TEMPERATURE scale with λ = √3")
        elif primary_p_temp == best_p:
            print("INTERPRETATION: Evidence for CGM chronology in TEMPERATURE scale with λ = 1/m_p")
        ...
        print("INTERPRETATION: No significant evidence for CGM chronology in any clock variable")
...
def monte_carlo_analysis_temperature(
    ...
    # Separate anchors from test milestones
    if exclude_anchors:
        test_milestones = [m for m in milestones if m.name not in ["Planck scale", "Today"]]
        anchor_milestones = [m for m in milestones if m.name in ["Planck scale", "Today"]]
    else:
        test_milestones = milestones
        anchor_milestones = []
    ...
    test_types = ["rayleigh_m2", "rayleigh_m3", "rayleigh_m4", "cgm_composite", "weighted_cgm", "kuiper", "watson"]
    for test_type in test_types:
        p_values = [s[test_type]["p_value"] for s in test_stats if test_type in s]
        concentrations = [s[test_type].get("concentration", 0.0) for s in test_stats if test_type in s]
...
def monte_carlo_analysis_scale_factor(
    ...
    results: List[ResultRow] = []
    for m in test_milestones:
        n_mean = recursion_depth_from_scale_factor(m.t_seconds, reference_scale_factor, m_p)
        k_int, d_int = nearest_integer(n_mean)
        k_half, d_half = nearest_half_integer(n_mean)
