...
def compute_tw_holonomy(m: int, n: int, k: float, theta_dom: float, signed: bool = True) -> float:
    """
...
def fit_triad(La: float, Lb: float, Lc: float, Nmax: int = 12) -> Dict[str, Any]:
    ...
    best = {
        "m": 1, "n": 1,
        "r_ab": np.exp(d1), "r_bc": np.exp(d2),
        "r_hat": np.exp(0.5 * (d1 + d2)), "err": float('inf')
    }
...
class TriadIndexAnalyzer:
    ...
    def constants(self) -> Dict[str, float]:
        ...
        # Atomic scales (QED)
        a0 = 4 * np.pi * self.epsilon0 * self.hbar**2 / (self.m_e * self.e**2)  # Bohr radius
        r_e = self.e**2 / (4 * np.pi * self.epsilon0 * self.m_e * self.c**2)    # Classical electron radius
        ...
        return {
            "lambda_e": L_e, "lambda_p": L_p, "lambda_H": L_H,
            "lambda_mu": L_mu, "lambda_tau": L_tau,
            "lambda_Z": L_Z, "lambda_W": L_W, "lambda_pi": L_pi,
            "a0": a0, "r_e": r_e, "L_cmb": L_cmb, "L_wien": L_wien,
            "dna_pitch": dna_pitch, "dna_rise": dna_rise, "dna_diameter": dna_diameter,
            "lambda_bar_e": lambda_bar_e, "sigma_T": sigma_T  # Added derived constants
        }
    ...
    def triad_report(self, name: str, La: float, Lb: float, Lc: float, 
                     Nmax: int = 12, domain: str = "unknown") -> Dict[str, Any]:
        """
    ...
    def calibrate_domains(self, reports: List[Dict[str, Any]]) -> None:
        ...
        for report in reports:
            ...
            elif "photon" in name and "photon_only" not in name and abs(delta_dom) < 0.5:
            ...
            # Use relativistic triads to calibrate unified GR domain
                ...
                if "relativistic_GR" not in self.domain_factors:
                    ...
                    existing["reference_delta"] = (existing["reference_delta"] * existing["count"] + delta_dom) / (existing["count"] + 1)
                    existing["count"] += 1
                    print(f"   Updated GR domain average: {existing['reference_delta']:.6f} (from {existing['count']} triads)")
    ...
    def fit_tw_parameters(self, reports: List[Dict[str, Any]]) -> None:
        ...
        for domain_name, domain_data in self.domain_factors.items():
            ...
            domain_triads = [r for r in reports if r.get("domain") == domain_name and "error" not in r]
    ...
    def test_out_of_sample(self, reports: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        ...
        for report in reports:
            ...
            if domain in self.domain_factors:              # exact match wins
                ...
                candidates = [d for d in self.domain_factors if d in domain or domain in d]
    ...
    def run(self) -> Dict[str, Any]:
        ...
        particle_triad = self.triad_report(
            "particle:H→p→e",
            K["lambda_H"], K["lambda_p"], K["lambda_e"],
            domain="particle"
        )
        reports.append(particle_triad)
        
        if "error" not in particle_triad:
            print(f"   LSI: {particle_triad['LSI']:.3f} ≈ {particle_triad['LSI_rational']}")
            print(f"   Integer purity: |LSI - p/q| = {abs(particle_triad['LSI'] - eval(particle_triad['LSI_rational'])):.6f}")
        ...
        print(f"   Scales: a0 = {K['a0']:.2e}, λe = {K['lambda_e']:.2e}, re = {K['r_e']:.2e}")
        atomic_triad = self.triad_report(
            "atomic:a0→λe→re",
            K["r_e"], K["lambda_e"], K["a0"],  # re < λe < a0 (ascending)
            domain="QED"
        )
        ...
        if "error" not in atomic_triad:
            ...
            print(f"   Integer purity: |LSI - p/q| = {abs(atomic_triad['LSI'] - eval(atomic_triad['LSI_rational'])):.6f}")
        ...
        print(f"   Scales: 550nm = {550e-9:.2e}, CMB = {K['L_cmb']:.2e}, Wien = {K['L_wien']:.2e}")
        photon_triad = self.triad_report(
            "photon:550nm→CMB→Wien",
            550e-9, K["L_cmb"], K["L_wien"],  # 550nm < CMB < Wien (ascending)
            domain="photon"
        )
        ...
        if "error" not in photon_triad:
            ...
            print(f"   Integer purity: |LSI - p/q| = {abs(photon_triad['LSI'] - eval(photon_triad['LSI_rational'])):.6f}")
        ...
        lepton_ladder = self.triad_report(
            "lepton_ladder:τ→μ→e",
            K["lambda_tau"], K["lambda_mu"], K["lambda_e"],  # λτ < λμ < λe (ascending)
            domain="lepton_ladder"
        )
        reports.append(lepton_ladder)
        
        if "error" not in lepton_ladder:
            print(f"   LSI: {lepton_ladder['LSI']:.3f} ≈ {lepton_ladder['LSI_rational']}")
            print(f"   Integer purity: |LSI - p/q| = {abs(lepton_ladder['LSI'] - eval(lepton_ladder['LSI_rational'])):.6f}")
        ...
        print(f"   Scales: λp = {K['lambda_p']:.2e}, λπ = {K['lambda_pi']:.2e}, λe = {K['lambda_e']:.2e}")
        qcd_triad = self.triad_report(
            "QCD:λp→λπ→λe",
            K["lambda_p"], K["lambda_pi"], K["lambda_e"],  # λp < λπ < λe (ascending)
            domain="QCD"
        )
        ...
        if "error" not in qcd_triad:
            ...
            print(f"   Integer purity: |LSI - p/q| = {abs(qcd_triad['LSI'] - eval(qcd_triad['LSI_rational'])):.6f}")
        ...
        frame_dragging_triad = self.triad_report(
            ...
            geodetic_effect, frame_dragging, total_relativistic,  # ascending order
        ...
        if "error" not in frame_dragging_triad:
            print(f"   LSI: {frame_dragging_triad['LSI']:.3f} ≈ {frame_dragging_triad['LSI_rational']}")
            print(f"   Integer purity: |LSI - p/q| = {abs(frame_dragging_triad['LSI'] - eval(frame_dragging_triad['LSI_rational'])):.6f}")
            ...
            print(f"   Frame-dragging/Geodetic ratio: {frame_dragging/geodetic_effect:.6f}")
        else:
        ...
        for domain, deltas in domain_groups.items():
            if deltas:
                ...
                print(f"  {domain}: mean = {delta_mean:.6f} {delta_sign}, std = {delta_std:.6f}, RMS = {delta_rms:.6f}")
                
                # Show TW parameters if available
                if domain in self.domain_factors:
                    params = self.domain_factors[domain]
                    print(f"    TW parameters: k={params['k']:.3f}, θ={np.degrees(params['theta_dom']):.1f}°, γ={params['gamma_dom']:.3f}")
        ...
        for domain, deltas in domain_groups.items():
            if len(deltas) > 1:
                ...
                for i, report in enumerate(reports):
                    if (report.get("domain") == domain and 
                        "error" not in report and 
                        "delta_dom" in report):
                        if abs(report["delta_dom"] - mean_delta) > 2 * std_delta:
                            outliers.append(f"{report['triad']}: δ = {report['delta_dom']:.6f}")
    ...
    def validate_tw_implementation(self) -> Dict[str, Any]:
        ...
        omega_expected = 0.5 * (k_small * m_small) * (k_small * n_small) * np.sin(theta_small)
        ...
        try:
            ...
            print(f"  Status: {'✅ PASS' if test_results['gyration_validation']['passed'] else '❌ FAIL'}")
        ...
        try:
            ...
            cgm_passed = relative_error < 0.1  # Accept up to 10% deviation like working codebase
            ...
            print(f"  Note: ~8% deviation is expected and indicates kinematic relationship")
    ...
    def anchor_sweep_analysis(self, reports: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        anchors = {
            ...
            "lambda_bar_e": self.constants()["lambda_bar_e"],  # Electron Compton (theory prior)
        ...
        for anchor_name, anchor_value in anchors.items():
            ...
            for triad in reports:
                ...
                expected_log = np.log(anchor_value) + N_i_star * np.log(Pi_loop) + delta_dom
        ...
        # Find best anchor
        if anchor_results:
            best_anchor = min(anchor_results.keys(), 
                            key=lambda k: anchor_results[k]["rms_error"])
            ...
            sorted_anchors = sorted(anchor_results.items(), 
                                  key=lambda x: x[1]["rms_error"])
            for i, (name, data) in enumerate(sorted_anchors, 1):
                marker = "🥇" if i == 1 else "🥈" if i == 2 else "🥉"
                print(f"  {marker} {name}: RMS={data['rms_error']:.6f}, Mean={data['mean_error']:.6f}")
