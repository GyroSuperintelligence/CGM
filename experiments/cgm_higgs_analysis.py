...
class CGMInvariants:
    ...
    def __post_init__(self):
        ...
        object.__setattr__(self, 'K_QG', self.Q_G * S_min)  # Quantum gravity coupling: Q_G × S_min ≈ 3.9374
        object.__setattr__(self, 'loop_factor', 1/(16*pi**2))  # Loop factor for RGE calculations
...
class StandardModelData:
    ...
    g1_MZ: float = 0.3583 * (5/3)**0.5  # U(1)_Y coupling (converted to GUT-normalized)
    g2_MZ: float = 0.6520               # SU(2)_L coupling  
    g3_MZ: float = 1.2177               # SU(3)_C coupling
    
    def __post_init__(self):
        """Initialize fermion masses."""
        object.__setattr__(self, 'm_fermions', {
            'electron': 0.000511, 'muon': 0.1057, 'tau': 1.777,
            'up': 0.0022, 'charm': 1.28, 'top': 173.0,
...
class SMRenormalizationGroup:
    ...
    def beta_lambda_2loop(self, λ: float, y_t: float, g1: float, g2: float, g3: float, use_two_loop: bool = True) -> float:
        """2-loop beta function for Higgs quartic coupling."""
        # 1-loop contribution (GUT-normalized g1)
        beta_1 = (24*λ**2 - 6*y_t**4 + (9/8)*g2**4 + (9/20)*g2**2*g1**2 + (27/200)*g1**4 +
        ...
        beta_2 = -78*λ**3 - 18*λ**2*y_t**2 + 54*λ*y_t**4 - 30*y_t**6 + \
                 λ**2*(9*g2**2 + 3*g1**2) - λ*y_t**2*(9*g2**2 + 3*g1**2 - 16*g3**2)
    ...
    def beta_y_top_1loop(self, y_t: float, g1: float, g2: float, g3: float) -> float:
        ...
        return (1/(16*pi**2)) * y_t * (9*y_t**2/2 - 17*g1**2/20 - 9*g2**2/4 - 8*g3**2)
    ...
    def beta_y_top_2loop(self, y_t: float, g1: float, g2: float, g3: float) -> float:
        ...
        beta_1 = y_t * ((9/2)*y_t**2 - 8*g3**2 - (9/4)*g2**2 - (17/20)*g1**2)

        if self.order == '1loop':
            return (1/(16*pi**2)) * beta_1

        # 2-loop (dominant terms)
        beta_2 = y_t * (-12*y_t**4 + (1187*g1**4)/216 + g1**2*g2**2/4 + (23*g2**4)/4)

        return (1/(16*pi**2)) * beta_1 + (1/(16*pi**2))**2 * beta_2
    
    def beta_gauge_couplings(self, g1: float, g2: float, g3: float, use_two_loop: bool = True) -> Tuple[float, float, float]:
        ...
        beta_g1_2 = (1/(16*pi**2))**2 * g1**3 * (199/50 * g1**2 + 27/10 * g2**2 + 44/5 * g3**2)
        beta_g2_2 = (1/(16*pi**2))**2 * g2**3 * (9/10 * g1**2 + 35/6 * g2**2 + 12 * g3**2)
        beta_g3_2 = (1/(16*pi**2))**2 * g3**3 * (11/10 * g1**2 + 9/2 * g2**2 - 26 * g3**2)
        
        return beta_g1_1 + beta_g1_2, beta_g2_1 + beta_g2_2, beta_g3_1 + beta_g3_2
    
    def evolve_couplings(self, mu_initial: float, mu_final: float,
                        lambda_initial: float, y_t_initial: float, use_two_loop: bool = True,
                        g1_init: Optional[float] = None, g2_init: Optional[float] = None, g3_init: Optional[float] = None) -> Tuple[float, float, float, float, float]:
        ...
        for i in range(len(log_scales) - 1):
            ...
            beta_g1, beta_g2, beta_g3 = self.beta_gauge_couplings(g1, g2, g3, use_two_loop)
            k1_g1, k1_g2, k1_g3 = dt * beta_g1, dt * beta_g2, dt * beta_g3
            
            k2_λ = dt * self.beta_lambda_2loop(λ + k1_λ/2, y_t + k1_y/2, g1 + k1_g1/2, g2 + k1_g2/2, g3 + k1_g3/2)
            k2_y = dt * self.beta_y_top_2loop(y_t + k1_y/2, g1 + k1_g1/2, g2 + k1_g2/2, g3 + k1_g3/2)
            beta_g1, beta_g2, beta_g3 = self.beta_gauge_couplings(g1 + k1_g1/2, g2 + k1_g2/2, g3 + k1_g3/2, use_two_loop)
            k2_g1, k2_g2, k2_g3 = dt * beta_g1, dt * beta_g2, dt * beta_g3
            
            k3_λ = dt * self.beta_lambda_2loop(λ + k2_λ/2, y_t + k2_y/2, g1 + k2_g1/2, g2 + k2_g2/2, g3 + k2_g3/2)
            k3_y = dt * self.beta_y_top_2loop(y_t + k2_y/2, g1 + k2_g1/2, g2 + k2_g2/2, g3 + k2_g3/2)
            beta_g1, beta_g2, beta_g3 = self.beta_gauge_couplings(g1 + k2_g1/2, g2 + k2_g2/2, g3 + k2_g3/2, use_two_loop)
            k3_g1, k3_g2, k3_g3 = dt * beta_g1, dt * beta_g2, dt * beta_g3
            
            k4_λ = dt * self.beta_lambda_2loop(λ + k3_λ, y_t + k3_y, g1 + k3_g1, g2 + k3_g2, g3 + k3_g3)
            k4_y = dt * self.beta_y_top_2loop(y_t + k3_y, g1 + k3_g1, g2 + k3_g2, g3 + k3_g3)
            beta_g1, beta_g2, beta_g3 = self.beta_gauge_couplings(g1 + k3_g1, g2 + k3_g2, g3 + k3_g3, use_two_loop)
            ...
            if (abs(λ) > coupling_thresholds['λ'] or
                abs(y_t) > coupling_thresholds['y_t'] or
                abs(g1) > coupling_thresholds['g1'] or
                abs(g2) > coupling_thresholds['g2'] or
                abs(g3) > coupling_thresholds['g3']):
                print(f"  Warning: Numerical instability at μ = {mu2:.2e} GeV")
                print(f"    Couplings: λ={λ:.2e}, y_t={y_t:.2e}, g1={g1:.2e}, g2={g2:.2e}, g3={g3:.2e}")
    ...
    def validate_rge(self, use_two_loop: bool = True) -> Dict[str, Any]:
        ...
        print(f"  Initial: μ = {mu_mt:.1f} GeV, λ = {λ_mt_init:.6f}, y_t = {y_t_mt_init:.6f}")
        ...
        λ_mt, y_t_mt, g1_mt, g2_mt, g3_mt = self.evolve_couplings(
            mu_mt, mu_mt, λ_mt_init, y_t_mt_init, use_two_loop, g1_mt_init, g2_mt_init, g3_mt_init
        )
...
class HiggsQuarticAnalysis:
    ...
    def predict_mh_from_uv(self, mu_uv: float, mu_ir: float = 173.0, use_two_loop: bool = False) -> Dict[str, Any]:
        """Predict Higgs mass from CGM UV boundary by evolving λ(E0) → μ_IR."""
        g1_uv, g2_uv, g3_uv, y_t_uv = self._evolve_couplings_to_uv(mu_uv, use_two_loop=use_two_loop)
    ...
    def find_ratio_crossing(self, target: float, kind: str = 'lambda_over_yt2',
        ...
        for i in range(n-1):
            ...
            val_i = lam_i/(y_i*y_i) if kind=='lambda_over_yt2' else lam_i/(g2_i*g2_i + g1_i*g1_i/3.0)
    ...
    def predict_coupling_ratios_at_intermediate_scale(self, mu_intermediate: float = 1e6) -> Dict[str, Union[str, float]]:
        ...
        try:
            lambda_int, _ = self.evolve_couplings_with_initial_couplings(
                91.1876, mu_intermediate, self.sm.lambda_SM, 
                0.94 * sqrt(2) * self.sm.m_fermions['top'] / self.sm.v_weak,
                self.sm.g1_MZ, self.sm.g2_MZ, self.sm.g3_MZ, use_two_loop=False
            )
            if lambda_int <= 0:
        ...
        measured_gauge_ratio = lambda_int / (g2_int**2 + g1_int**2/3) if (g2_int**2 + g1_int**2/3) > 0 else float('inf')
        ...
        log_scale_factor = log(mu_intermediate / 91.1876) / log(self.cgm.E0_reciprocal / 91.1876)
        ...
        return {
            ...
            'yukawa_deviation': abs(measured_yukawa_ratio - cgm_lambda_uv) / cgm_lambda_uv,
            'gauge_deviation': abs(measured_gauge_ratio - cgm_lambda_uv) / cgm_lambda_uv,
            'log_scale_factor': log_scale_factor
        }

    def apply_aperture_corrections(self, coupling: float, generation: int = 1, correction_type: str = 'additive') -> float:
    ...
    def reformulate_higgs_quartic_with_corrections(self) -> Dict[str, float]:
        ...
        formulations = {
            ...
            'monodromy': lambda_base * abs(cmath.exp(1j * self.cgm.phi_SU2)) * (1 - delta),
            'generational_1': self.apply_aperture_corrections(lambda_base, 1, 'generational'),
            'generational_2': self.apply_aperture_corrections(lambda_base, 2, 'generational'),
            'generational_3': self.apply_aperture_corrections(lambda_base, 3, 'generational')
        }
    ...
    def _evolve_couplings_to_uv(self, mu_uv: float, use_two_loop: bool = False) -> Tuple[float, float, float, float]:
        ...
        for i in range(1, n_points):
            ...
            if use_two_loop:
                ...
                beta_g1, beta_g2, beta_g3 = rge.beta_gauge_couplings(g1, g2, g3, use_two_loop)
            else:
    ...
    def evolve_couplings_with_initial_couplings(self, mu_initial: float, mu_final: float,
                                               lambda_initial: float, y_t_initial: float,
                                               g1_initial: float, g2_initial: float, g3_initial: float,
                                               use_two_loop: bool = True) -> Tuple[float, float]:
        """Phase B: Evolve λ and y_t from μ_UV to m_H using specific initial gauge couplings."""
        # Unified call to SM RGE geometric stepper
        rge = SMRenormalizationGroup(self.sm, order='1loop' if not use_two_loop else '2loop')
        λ_out, y_t_out, _, _, _ = rge.evolve_couplings(
            mu_initial, mu_final, lambda_initial, y_t_initial,
            use_two_loop=use_two_loop,
            g1_init=g1_initial, g2_init=g2_initial, g3_init=g3_initial
        )
    ...
    def analyze_yukawa_slope_by_sector(self) -> Dict[str, Dict[str, float]]:
        ...
        for name, fermions in sectors.items():
            if all(f in yukawas for f in fermions):
                ...
                print(f"  {name:10s}: slope={slope:.3f}, slope/log(1/Δ)={slope_ratio:.3f}")
    ...
    def validate_cgm_predictions(self, cgm_predictions: Dict) -> Dict:
        ...
        if 'sm_compatibility' in cgm_predictions:
            validation['compatibility'] = {
                ...
                'difference': abs(cgm_predictions['sm_compatibility'] - self.sm_compatibility)
            }
...
def main():
    ...
    if sm.m_fermions and 'top' in sm.m_fermions:
        print(f"SM parameters: m_t = {sm.m_fermions['top']:.1f} GeV, λ_SM = {sm.lambda_SM:.5f} at μ ~ m_t")
    else:
    ...
    if 'error' not in validation_2loop:
        print(f"  1-loop λ(m_t): {validation_1loop.get('lambda_mt_predicted', 'N/A'):.6f}")
    ...
    print(f"CGM UV→IR Higgs test (1-loop): m_H(pred) = {res_1loop['mH_pred']:.2f} GeV from λ(E0) = {res_1loop['lambda_uv']:.6f}")
    ...
    resF_1loop = higgs_analysis.predict_mh_from_uv(cgm.E0_forward, 173.0, use_two_loop=False)
    resF_2loop = higgs_analysis.predict_mh_from_uv(cgm.E0_forward, 173.0, use_two_loop=True)
    print(f"E0_forward (1-loop): m_H(pred) = {resF_1loop['mH_pred']:.2f} GeV from λ(E0) = {resF_1loop['lambda_uv']:.6f}")
    print(f"E0_forward (2-loop): m_H(pred) = {resF_2loop['mH_pred']:.2f} GeV from λ(E0) = {resF_2loop['lambda_uv']:.6f}")
    ...
    for eps in [-0.01, 0.0, +0.01]:
        g1, g2, g3, y = higgs_analysis._evolve_couplings_to_uv(cgm.E0_reciprocal, use_two_loop=False)
        res = higgs_analysis.evolve_couplings_with_initial_couplings(
            cgm.E0_reciprocal, 173.0, higgs_analysis.compute_uv_boundary_lambda(),
            y*(1+eps), g1, g2, g3, use_two_loop=False
        )
    ...
    for k in ['lambda_over_yt2', 'lambda_over_g']:
        hit_1loop = higgs_analysis.find_ratio_crossing(cgm_uv, kind=k, mu_min=91.2, mu_max=1e16, use_two_loop=False)
        hit_2loop = higgs_analysis.find_ratio_crossing(cgm_uv, kind=k, mu_min=91.2, mu_max=1e16, use_two_loop=True)
    ...
    hit_yt2 = higgs_analysis.find_ratio_crossing(cgm_uv, kind='lambda_over_yt2', mu_min=91.2, mu_max=1e16, use_two_loop=True)
    hit_g = higgs_analysis.find_ratio_crossing(cgm_uv, kind='lambda_over_g', mu_min=91.2, mu_max=1e16, use_two_loop=True)
    ...
    if hit_yt2['mu_star'] and hit_g['mu_star']:
        ...
        _, y_t_yt2, g1_yt2, g2_yt2, g3_yt2 = rge_2loop.evolve_couplings(91.1876, mu_yt2, 0.126, 0.94, True, 0.463, 0.648, 1.167)
        _, y_t_g, g1_g, g2_g, g3_g = rge_2loop.evolve_couplings(91.1876, mu_g, 0.126, 0.94, True, 0.463, 0.648, 1.167)
        ...
        # Average μ* and check
        mu_avg = (mu_yt2 * mu_g)**0.5
        _, y_t_avg, g1_avg, g2_avg, g3_avg = rge_2loop.evolve_couplings(91.1876, mu_avg, 0.126, 0.94, True, 0.463, 0.648, 1.167)
        ...
        print(f"  2 - Δ = {2 - cgm.Delta:.6f} (error to μ ratio*: {abs(mu_ratio - (2 - cgm.Delta))/ (2 - cgm.Delta) * 100:.2f}%)")
    ...
    for mu_int in intermediate_scales:
        ratio_pred = higgs_analysis.predict_coupling_ratios_at_intermediate_scale(mu_int)
        ...
        if 'error' not in ratio_pred:
            ...
            print(f"    Measured Yukawa ratio λ/y_t² = {ratio_pred['measured_yukawa_ratio']:.6f}")
            print(f"    Distance to CGM invariant: ΔR_y = {ratio_pred['yukawa_deviation']:.1%}")
            print(f"    Measured Gauge ratio λ/(g²+g²/3) = {ratio_pred['measured_gauge_ratio']:.6f}")
            print(f"    Distance to CGM invariant: ΔR_g = {ratio_pred['gauge_deviation']:.1%}")
            print()
    ...
    for name, corr_type, gen in corrections:
        corrected = higgs_analysis.apply_aperture_corrections(lambda_base, gen, corr_type)
    ...
    for name, lambda_value in formulations.items():
        error = abs(lambda_value - target_lambda) / target_lambda if target_lambda > 0 else float('inf')
    ...
    variants = {
        ...
        'aperture-': (lam_SM*(1-cgm.Delta), lam3_SM*(1-cgm.Delta)),
        'monodromy': (lam_SM*abs(cmath.exp(1j*cgm.phi_SU2)), lam3_SM*abs(cmath.exp(1j*cgm.phi_SU2)))
    }
    ...
    print(f"  Note: This is a structural check of CGM terms, not a phenomenological mass prediction")
    ...
    if sm.m_fermions is not None:
        for fermion in ['electron', 'muon', 'tau', 'up', 'charm', 'top', 'down', 'strange', 'bottom']:
    ...
    if 'error' not in slope_analysis:
        ...
        print(f"  Correlation with log(1/Δ): {slope_analysis['correlation_with_1_delta']}")
        print(f"  Robustness band (vary masses ±1%):")
        if 'robustness_band' in slope_analysis:
            band = slope_analysis['robustness_band']
            print(f"    Slope ratio band: [{band['ratio_min']:.3f}, {band['ratio_max']:.3f}]")
        print(f"  Geometric ratios:")
        for name, value in slope_analysis['geometric_ratios'].items():
            ratio = abs(slope_analysis['observed_slope'] / value) if value != 0 else float('inf')
    ...
    print(f"  CGM: {BR_mumu_BR_tautau_CGM:.6f} (shift: {(BR_mumu_BR_tautau_CGM/BR_mumu_BR_tautau_SM - 1)*100:+.2f}%)")
    
    print(f"BR(cc)/BR(bb) predictions:")
    print(f"  SM: {BR_cc_BR_bb_SM:.6f}")
    print(f"  CGM: {BR_cc_BR_bb_CGM:.6f} (shift: {(BR_cc_BR_bb_CGM/BR_cc_BR_bb_SM - 1)*100:+.2f}%)")
    
    print(f"BR(ττ)/BR(bb) predictions:")
    print(f"  SM: {BR_tautau_BR_bb_SM:.6f}")
    print(f"  CGM: {BR_tautau_BR_bb_CGM:.6f} (shift: {(BR_tautau_BR_bb_CGM/BR_tautau_BR_bb_SM - 1)*100:+.2f}%)")
    
    print(f"\nNote: CGM generational pattern κ_f(gen) = (1 + Δ)^(gen-1)")
    print(f"      Δ = {cgm.Delta:.4f} drives correlated shifts across all Higgs couplings")
    ...
    print(f"  CGM BR = {BR_mumu_CGM:.2e} ({(BR_mumu_CGM/BR_mumu_SM-1)*100:+.1f}% up from SM)")
    ...
    print(f"\nH→Zγ excess (2.5σ):")
    print(f"  CGM prediction: ~{rate_shift*100:.1f}% rate deviation from aperture-scaled monodromy")
    ...
    print(f"  Higgs mass: {experimental.higgs_mass:.2f} GeV (precision: {experimental.mass_precision:.1%})")
    print(f"  SM compatibility: {experimental.sm_compatibility:.1%}")
    print(f"  Tree-level λ = m_H²/(2 v²) = {(experimental.higgs_mass**2)/(2*sm.v_weak**2):.6f} (target: {sm.lambda_SM:.6f})")
    ...
    if 'leptons' in sector_analysis:
        ...
        print(f"  🎯 Leptonic Yukawa slope/log(1/Δ): {leptonic_slope_ratio:.3f} (target 1.0)")
        print(f"     → This is a clean 'dimensional' success because Yukawas are dimensional through v_weak")
