...
def bh_properties(name: str, M_kg: float) -> BHResult:
    ...
    return BHResult(
        name=name, M_kg=M_kg, M_solar=M_kg / M_sun,
        r_s_m=r_s, A_m2=A,
        S_BH_J_per_K=S_BH, S_CGM_J_per_K=S_CGM, S_factor=(1.0 + m_p),
        T_H_K=T_H, T_CGM_K=T_CGM, T_factor=1.0 / (1.0 + m_p),
        tau_std_s=tau_std, tau_cgm_s=tau_cgm, tau_factor=(1.0 + m_p)**4
    )
...
def print_result(res: BHResult) -> None:
    ...
    print(f"  S_CGM: {fmt_si(res.S_CGM_J_per_K, 'J/K')}  (×{res.S_factor:.6f},  +{(res.S_factor-1.0)*100:.4f}%)")
    print(f"  S_BH/k_B  = {res.S_BH_J_per_K/k_B:.3e}")
    print(f"  S_CGM/k_B = {res.S_CGM_J_per_K/k_B:.3e}  (bits: {(res.S_CGM_J_per_K/(k_B*math.log(2))):.3e})")
    print("Temperature (standard → CGM):")
    print(f"  T_H:   {fmt_si(res.T_H_K, 'K')}")
    print(f"  T_CGM: {fmt_si(res.T_CGM_K, 'K')}  (×{res.T_factor:.6f},  −{(1.0-res.T_factor)*100:.4f}%)")
...
def print_derived_predictions() -> None:
    ...
    print(f"   Total quanta ratio: N_total,CGM / N_total,std = (1 + m_p) = {N_ratio:.6f}")
    print(f"   Interpretation: More quanta, each with lower energy")
    
    print(f"\n6. Page time and Page mass:")
    t_page_ratio = (1.0 + m_p)**4
    print(f"   Page time ratio: t_Page,CGM / t_Page,std = (1 + m_p)^4 = {t_page_ratio:.6f}")
    ...
    print(f"   Redshift factor: {1.0 - E_peak_ratio:.4f} = {(1.0 - E_peak_ratio)*100:.2f}% redward")
...
def print_horizon_micro_quanta() -> None:
    ...
    print(f"ℓ_P,eff (from G_eff) = {fmt_si(lP_eff, 'm')}  (×{1.0/math.sqrt(1.0 + m_p):.6f})")
...
def print_desitter_horizon_scaling() -> None:
    ...
    print(f"S_dS,CGM = (1 + m_p) S_dS,std = {fmt_si(S_dS_cgm, 'J/K')}  (×{1.0 + m_p:.6f})")
...
def print_binary_merger_analysis() -> None:
    ...
    print(f"  • Remnant lifetime: Extended by factor (1 + m_p)^4 ≈ {lifetime_factor:.3f}")
    ...
    print(f"  Standard lifetime: {seconds_to_readable((5120.0 * pi * G**2 * M_remnant**3) / (hbar * c**4))}")
    print(f"  CGM lifetime: {seconds_to_readable((5120.0 * pi * G**2 * M_remnant**3) / (hbar * c**4) * lifetime_factor)}")
...
def print_ads_blackhole_analysis() -> None:
    ...
    T_ads_std = (hbar * c / (4.0 * pi * k_B)) * (1.0/r_plus_approx + 3.0 * r_plus_approx / (L_ads**2))
...
def print_kerr_newman_example():
    ...
    print(f"Spin length: a = {fmt_si(kn['a_len'], 'm')}  (a_* = {kn['a_len']/kn['r_g']:.3f})")
    print(f"Horizon radii: r_+ = {fmt_si(kn['r_plus'], 'm')},  r_- = {fmt_si(kn['r_minus'], 'm')}")
    ...
    print(f"  Note: Kerr–Newman geometry (Ω_H, Φ_H, QNM spectrum) unchanged by CGM scaling")
...
if __name__ == "__main__":
    ...
    print("Assumption: S_CGM = S_BH × (1 + m_p), m_p = 1/(2*sqrt(2*pi)) ≈ {:.12f}".format(m_p))
