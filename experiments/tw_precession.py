...
def batch_check(n: int = 200, beta_max: float = 0.1, seed: int = 0, c: float = 1.0) -> Dict[str, Any]:
    """
...
def slope_check(n: int = 400, beta_max_list: tuple = (0.02, 0.03, 0.05, 0.08, 0.10), seed: int = 0, c: float = 1.0) -> List[Dict[str, Any]]:
    """
...
def main():
    ...
    rows = slope_check()
    for r in rows:
        print(f"  β≤{r['beta_max']:.2f}: slope={r['slope']:.6f} max|res|={r['max_abs_resid']:.2e}")
