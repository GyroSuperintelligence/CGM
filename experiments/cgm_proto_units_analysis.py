...
def compute_polygon_recursion(iterations: int) -> List[PolygonIteration]:
    ...
    for _ in range(iterations):
        ...
        circumscribed = (2 * circumscribed) / (
            mp.sqrt(4 + circumscribed * circumscribed) + 2
        )
        sides *= 2
...
def check_iteration_scaling(
    prev: PolygonIteration, curr: PolygonIteration
) -> Dict[str, Any]:
    """
    ...
    try:
        amplitude_ratio = (
            curr.quantum_amplitude / prev.quantum_amplitude
            if prev.quantum_amplitude != 0
            else mp.mpf("inf")
        )
    except:
...
def calibrate_physical_units(
    ...
    gravity_recovery = (
        gravitational_coupling * length_scale**3 / (mass_scale * time_scale**2)
    )
...
def compute_tautological_relations(pi_value: Any) -> Dict[str, Dict[str, Any]]:
    ...
    return {
        "from_solid_angle": {"formula": "π = Q_G / 4", "value": solid_angle / 4},
        "from_horizon": {
            "formula": "π = L_horizon² / 2",
            "value": (horizon * horizon) / 2,
        },
        "from_cavity": {"formula": "π = Q_cavity² / 8", "value": (cavity * cavity) / 8},
        "from_action": {"formula": "π = 32 × S_min²", "value": 32 * (action * action)},
        "exact_relations": {
...
def analyze_scale_relationships(
    geometric_units: CGMGeometricUnits, calibration: Dict[str, Any]
) -> Dict[str, Any]:
    """
    ...
    pi_value = mp.pi
    reciprocal_action = (
        2 * pi_value - pi_value / 2
    ) * geometric_units.aperture_parameter
...
def validate_unit_independence(geometric_units: CGMGeometricUnits) -> Dict[str, Any]:
    ...
    for test_zeta in test_zetas:
        ...
        cal_forward = calibrate_physical_units(
            geometric_units, gravitational_coupling=test_zeta, use_reciprocal=False
        )
        cal_reciprocal = calibrate_physical_units(
            geometric_units, gravitational_coupling=test_zeta, use_reciprocal=True
        )
...
def calculate_fine_structure_constant(
    ...
    return {
        ...
        "status": (
            "excellent"
            if deviation < 0.001
            else "very_good" if deviation < 0.01 else "good"
        ),
...
def show_theoretical_predictions(
    minimal_action: Any, gravitational_coupling: Optional[Any] = None
) -> Dict[str, Any]:
    """
...
def print_comprehensive_results(
    ...
    print(
        f"   Final precision: {mp.nstr(iterations[-1].relative_precision, 2)} (relative)"
    )
    ...
    if len(iterations) > 1:
        ...
        print(
            f"   Gap ratio: {mp.nstr(scaling['gap_ratio'], 6)} (expected asymptotically: 0.25)"
        )
        print(
            f"   Amplitude ratio: {mp.nstr(scaling['amplitude_ratio'], 6)} (expected asymptotically: 0.25)"
        )
        print(f"   Gap validation: {'✓' if scaling['gap_valid'] else '✗'}")
        print(f"   Amplitude validation: {'✓' if scaling['amplitude_valid'] else '✗'}")
        print(
            f"   Convergence relationship: E_Q/S_min = {mp.nstr(iterations[-1].quantum_amplitude/geometric_units.minimal_action, 8)}"
        )
    ...
    print(
        f"   t_aperture = m_p = 1/(2√(2π)) = {mp.nstr(geometric_units.t_aperture, 10)}"
    )
    ...
    print(
        f"   S_geo = m_p×π×√3/2 = {mp.nstr(geometric_units.geometric_mean_action, 10)}"
    )
    print(
        f"   c_CGM = L_horizon/t_aperture = 4π = {mp.nstr(geometric_units.c_cgm, 10)} (geometric speed ratio)"
    )
    print(f"   ℰ_CGM = S_min/t_aperture = π/2 = {mp.nstr(geometric_units.E_unit, 10)}")
    print(
        f"   Q_G = 4π = {mp.nstr(geometric_units.Q_G, 10)} (complete solid angle / survey closure)"
    )
    ...
    print(
        f"   Q_G × m_p² = {mp.nstr(geometric_units.geometric_closure_factor, 10)} (fundamental relationship = 1/2)"
    )
    print(
        f"   m_p = {mp.nstr(geometric_units.aperture_parameter_value, 10)} (aperture parameter ≈ 20%)"
    )
    print(
        f"   Holonomy deficit = {mp.nstr(geometric_units.holonomy_deficit, 10)} rad (toroidal invariant)"
    )
    ...
    print(
        f"     c̄² = 1/(Q_G m_p²) = {mp.nstr(einstein_gauge['cbar2'], 6)} (→ c̄ = {mp.nstr(einstein_gauge['cbar'], 6)})"
    )
    print(f"     m̄ = Ē/c̄² = {mp.nstr(einstein_gauge['mbar'], 6)} (enforces E=mc²)")
    print(f"     p̄ = m̄ c̄ = {mp.nstr(einstein_gauge['pbar'], 6)}")
    print(
        f"     Closure identity: Q_G × m_p² = {mp.nstr(einstein_gauge['closure_identity'], 6)}"
    )
    print(
        f"     Einstein relation: m̄ × c̄² = {mp.nstr(einstein_gauge['einstein_relation'], 6)}"
    )

    # Section 4: Action Bridge
    print("\n4. ACTION BRIDGE: S_min × κ = ℏ")
    print(
        f"   S_min = {mp.nstr(action_bridge['fundamental_bridge']['S_min'], 10)} (dimensionless)"
    )
    print(f"   κ = {mp.nstr(action_bridge['fundamental_bridge']['kappa'], 4)} [J·s]")
    print(f"   ℏ = {mp.nstr(hbar_SI, 4)} [J·s]")
    print(
        f"   Bridge accuracy: {mp.nstr(action_bridge['verification']['bridge_accuracy'], 2)}"
    )
    print(
        f"   Verification: {'✓' if action_bridge['verification']['bridge_success'] else '✗'}"
    )
    ...
    print(
        f"   Gravitational coupling: ζ = {mp.nstr(gravitational_coupling, 8)} (assumption-free)"
    )
    ...
    print(
        f"   T₀ = {mp.nstr(calibration['T0'], 4)} ± {mp.nstr(T0_uncertainty, 2)} [s] per geometric time"
    )
    ...
    print(
        f"   L₀ = {mp.nstr(calibration['L0'], 4)} ± {mp.nstr(L0_uncertainty, 2)} [m] per geometric length"
    )
    print(
        f"   E₀ = {mp.nstr(calibration['E0'], 4)} ± {mp.nstr(E0_uncertainty, 2)} [J] per geometric energy"
    )
    print(
        f"   M₀ = {mp.nstr(calibration['M0'], 4)} ± {mp.nstr(M0_uncertainty, 2)} [kg] per geometric mass"
    )
    ...
    print(f"\n   Alternative units:")
    print(
        f"     E₀ = {mp.nstr(E0_GeV, 4)} ± {mp.nstr(E0_GeV_uncertainty, 2)} [GeV] = {mp.nstr(E0_eV, 4)} ± {mp.nstr(E0_eV_uncertainty, 2)} [eV]"
    )
    print(
        f"     L₀ = {mp.nstr(L0_fm, 4)} ± {mp.nstr(L0_fm_uncertainty, 2)} [fm] = {mp.nstr(L0_cm, 4)} ± {mp.nstr(L0_cm_uncertainty, 2)} [cm]"
    )
    ...
    G_recovered = (
        calibration["zeta"]
        * calibration["L0"] ** 3
        / (calibration["M0"] * calibration["T0"] ** 2)
    )
    ...
    print(
        f"     Forward energy: E₀ = {mp.nstr(uv_ir_analysis['dual_mode_analysis']['forward_energy'], 4)} [J]"
    )
    print(
        f"     Reciprocal energy: E₀ = {mp.nstr(uv_ir_analysis['dual_mode_analysis']['reciprocal_energy'], 4)} [J]"
    )
    print(
        f"     Energy ratio: {mp.nstr(uv_ir_analysis['dual_mode_analysis']['energy_ratio'], 6)}"
    )
    print(
        f"     √3 exact: {mp.nstr(uv_ir_analysis['dual_mode_analysis']['sqrt3_exact'], 6)}"
    )
    print(
        f"     Ratio accuracy: {mp.nstr(uv_ir_analysis['dual_mode_analysis']['ratio_accuracy'], 2)}"
    )
    print(
        f"     Signature verified: {'✓' if uv_ir_analysis['dual_mode_analysis']['signature_verified'] else '✗'}"
    )
    ...
    print(
        f"     Action ratio: {mp.nstr(uv_ir_analysis['action_analysis']['action_ratio'], 6)}"
    )

    print(f"   Survey coupling mechanism:")
    print(
        f"     Q_G = {mp.nstr(uv_ir_analysis['survey_coupling']['Q_G'], 10)} (survey constant)"
    )
    print(
        f"     m_p = {mp.nstr(uv_ir_analysis['survey_coupling']['m_p'], 10)} (aperture parameter)"
    )
    print(
        f"     Nonlocal kernel: {mp.nstr(uv_ir_analysis['survey_coupling']['nonlocal_kernel'], 10)}"
    )
    print(
        f"     Closure identity: {mp.nstr(uv_ir_analysis['survey_coupling']['closure_identity'], 10)}"
    )
    ...
    print(f"\n   Unit independence validation:")
    print(
        f"     √3 ratio preserved across different ζ values: {'✓' if unit_validation['validation_passed'] else '✗'}"
    )
    ...
    print(
        f"     ζ = Q_G/S_geo consistency: {'✓' if gauge_results['audit_passed'] else '✗'}"
    )
    ...
    cal_forward = calibrate_physical_units(
        geometric_units, gravitational_coupling, use_reciprocal=False
    )
    cal_reciprocal = calibrate_physical_units(
        geometric_units, gravitational_coupling, use_reciprocal=True
    )

    print(f"   Forward mode (standard):")
    print(
        f"     Action quantum: S_min = (π/2)×m_p = {mp.nstr(geometric_units.S_min, 10)}"
    )
    ...
    print(
        f"     Action quantum: S_reciprocal = (3π/2)×m_p = {mp.nstr(geometric_units.reciprocal_action, 10)}"
    )
    print(f"     Energy scale: E₀ = {mp.nstr(cal_reciprocal['E0'], 4)} [J]")
    print(
        f"     Energy scale: E₀ = {mp.nstr(cal_reciprocal['E0']/(e_SI*1e9), 4)} [GeV]"
    )
    ...
    predictions = show_theoretical_predictions(
        geometric_units.minimal_action, gravitational_coupling
    )
    ...
    print(
        f"     T₀/T_Planck = {mp.nstr(predictions['planck_ratios']['T0/T_Planck'], 6)}"
    )
    print(
        f"     L₀/L_Planck = {mp.nstr(predictions['planck_ratios']['L0/L_Planck'], 6)}"
    )
    print(
        f"     E₀/E_Planck = {mp.nstr(predictions['planck_ratios']['E0/E_Planck'], 6)}"
    )
    ...
    print(
        f"     E₀/E_Planck = {mp.nstr(scale_analysis['energy_ratios']['E0/E_Planck'], 4)}"
    )
    print(f"     E₀/E_GUT = {mp.nstr(scale_analysis['energy_ratios']['E0/E_GUT'], 4)}")
    print(
        f"     E₀/E_weak = {mp.nstr(scale_analysis['energy_ratios']['E0/E_weak'], 2)}"
    )

    print(f"   Geometric factor analysis:")
    print(f"     (4π)² = {mp.nstr(scale_analysis['geometric_factors']['(4π)²'], 4)}")
    print(
        f"     (4π)²/m_p = {mp.nstr(scale_analysis['geometric_factors']['(4π)²/m_p'], 4)}"
    )
    print(
        f"     1/S_min = {mp.nstr(scale_analysis['geometric_factors']['1/S_min'], 4)}"
    )

    print(f"   Reciprocal hypothesis:")
    print(
        f"     S_reciprocal/S_min = {mp.nstr(scale_analysis['reciprocal_analysis']['S_reciprocal/S_min'], 6)}"
    )
    print(
        f"     Expected energy ratio = {mp.nstr(scale_analysis['reciprocal_analysis']['expected_energy_ratio'], 6)} (should equal √(S_reciprocal/S_min))"
    )
    print(
        f"     Reciprocal contribution = {mp.nstr(scale_analysis['reciprocal_analysis']['reciprocal_contribution'], 6)}"
    )

    # Section 13: Key Mathematical Relationships
    print("\n13. KEY MATHEMATICAL RELATIONSHIPS")
    print(
        f"   Geometric completeness: Q_G = 4π = {mp.nstr(4*mp.pi, 10)} (complete solid angle)"
    )
    print(
        f"   Aperture closure: Q_G × m_p² = {mp.nstr(4*mp.pi * geometric_units.m_p**2, 10)} (expected: 0.5)"
    )
    ...
    print(
        f"   Horizon relationship: 4π × m_p = {mp.nstr(4*mp.pi * geometric_units.m_p, 10)} (equals L_horizon)"
    )
    print(
        f"   Energy consistency: κ/T₀ = E₀ = {mp.nstr(calibration['kappa']/calibration['T0'], 4)} [J]"
    )
    print(
        f"   Gravity verification: (Q_G/S_geo) × L₀³/(M₀T₀²) = {mp.nstr(G_SI, 4)} [m³/(kg·s²)]"
    )
    ...
    print(
        f"     T_Planck = {mp.nstr(planck_analysis['planck_scales']['T_Planck'], 4)} [s]"
    )
    print(
        f"     L_Planck = {mp.nstr(planck_analysis['planck_scales']['L_Planck'], 4)} [m]"
    )
    print(
        f"     E_Planck = {mp.nstr(planck_analysis['planck_scales']['E_Planck'], 4)} [J]"
    )

    print(
        f"   Raw geometric CGM-Planck conversion ratios (convention/consistency check):"
    )
    print(f"     (Using raw geometric scales: T_CGM = m_p × T_Planck, etc.)")
    print(
        f"     T_CGM/T_Planck = {mp.nstr(planck_analysis['conversion_ratios']['T_CGM/T_Planck'], 6)}"
    )
    print(
        f"     L_CGM/L_Planck = {mp.nstr(planck_analysis['conversion_ratios']['L_CGM/L_Planck'], 6)}"
    )
    print(
        f"     E_CGM/E_Planck = {mp.nstr(planck_analysis['conversion_ratios']['E_CGM/E_Planck'], 6)}"
    )
    ...
    print(
        f"     ℏ recovery: {'✓' if planck_analysis['verification']['hbar_recovery']['success'] else '✗'}"
    )
    print(
        f"     c recovery: {'✓' if planck_analysis['verification']['c_recovery']['success'] else '✗'}"
    )
    ...
    if show_tautologies:
        ...
        print(
            f"     Q_G × m_p² = {mp.nstr(relations['Q_G × m_p²'], 10)} (expected: 0.5)"
        )
        print(
            f"     4π × m_p = {mp.nstr(relations['4π × m_p'], 10)} (equals L_horizon)"
        )
    ...
    try:
        ...
        print(
            f"     Deviation = {mp.nstr(fine_structure_result['deviation'], 2)} ({mp.nstr(fine_structure_result['deviation_percent'], 4)}%)"
        )
        ...
        print(
            f"     δ_BU* = (α_CODATA × m_p)^(1/4) = {mp.nstr(fine_structure_result['delta_BU_star'], 8)} rad"
        )
        print(
            f"     |δ_BU - δ_BU*| = {mp.nstr(fine_structure_result['delta_BU_diff'], 2)} rad"
        )
    ...
    print(
        f"   Core geometric invariant: Q_G = 4π (complete solid angle / survey closure)"
    )
    ...
    print(
        f"   Aperture parameter: m_p = {mp.nstr(geometric_units.m_p, 10)} (aperture parameter ≈ 20%; 97.9% closure is conceptual/holonomy-based)"
    )
    print(
        f"   Geometric mean action: S_geo = {mp.nstr(geometric_units.geometric_mean_action, 10)} (dual-invariance compatible)"
    )
    ...
    print(
        f"   QG-4π unit system provides testable predictions for quantum gravitational phenomena"
    )
    print(
        f"   UV/IR mixing signature: √3 energy ratio between forward/reciprocal modes"
    )
...
def print_convergence_table(iterations: List[PolygonIteration]) -> None:
    ...
    for i in key_indices:
        it = iterations[i]
        print(
            f"{it.sides:12d} {format_number(it.lower_bound, 10):>15} "
    ...
    print(
        f"  Final relative precision: {mp.nstr(iterations[-1].relative_precision, 2)}"
    )
    print(
        f"  Monotonic convergence: {'✓' if all(iterations[i].gap > iterations[i+1].gap for i in range(len(iterations)-1)) else '✗'}"
    )
...
def main(
    ...
    if not iterations:
        print(
            f"  Warning: Target precision not achieved in {max_iterations} iterations"
        )
    ...
    print(
        f"  Energy scale set from final quantum amplitude: {mp.nstr(iterations[-1].quantum_amplitude, 8)}"
    )
